...
log = get_logger("session_manager", "session_manager.log")
...
class SessionManager:
    ...
    def _start(self, ts: _TrackState, track_id: int, frame: np.ndarray) -> None:
        ...
        ts.video_path = path
        h, w          = frame.shape[:2]
        ts.writer     = AsyncVideoWriter(path, self._fps, w, h, crf="26")
        ...
        try:
            ...
            log.error("DB error on start: %s", e)
    ...
    def _close(self, ts: _TrackState) -> None:
        ...
        if ts.writer:
            ...
            if ts.writer.dropped:
                log.warning("dropped %d frame(s)", ts.writer.dropped)
            ts.writer = None
            ...
            if too_short:
                ...
                if ts.video_path and os.path.exists(ts.video_path):
                    try:
                        ...
                        pass
                log.info("DISCARDED short session → %s", ts.video_path)
            else:
                try:
                    with get_session() as db:
                        row = db.get(UserSession, ts.db_session_id)
